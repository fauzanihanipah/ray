#!/bin/bash
# ============================================================
#   CHANELOG VPN SCRIPT - INSTALLER
#   Repository: https://github.com/chanelog/xray
# ============================================================

REPO="https://raw.githubusercontent.com/chanelog/bin/main"
RAW="https://raw.githubusercontent.com/fauzanihanipah/ray/main"
SCRIPT_DIR="/etc/vpn-script"
BIN_DIR="/usr/local/bin"

# ─── Colors ────────────────────────────────────────────────
RED='\033[0;31m'
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
CYAN='\033[0;36m'
PURPLE='\033[0;35m'
WHITE='\033[1;37m'
NC='\033[0m'

# ─── Check Root ────────────────────────────────────────────
if [[ $EUID -ne 0 ]]; then
  echo -e "${RED}[ERROR]${NC} Script harus dijalankan sebagai root!"
  exit 1
fi

# ─── Check OS ──────────────────────────────────────────────
. /etc/os-release 2>/dev/null
if [[ "$ID" != "ubuntu" && "$ID" != "debian" ]]; then
  echo -e "${RED}[ERROR]${NC} Script hanya mendukung Ubuntu/Debian!"
  exit 1
fi

# ─── Banner ────────────────────────────────────────────────
clear
echo -e "${CYAN}"
cat <<'EOF'
  ██████╗██╗  ██╗ █████╗ ███╗   ██╗███████╗██╗      ██████╗  ██████╗
 ██╔════╝██║  ██║██╔══██╗████╗  ██║██╔════╝██║     ██╔═══██╗██╔════╝
 ██║     ███████║███████║██╔██╗ ██║█████╗  ██║     ██║   ██║██║  ███╗
 ██║     ██╔══██║██╔══██║██║╚██╗██║██╔══╝  ██║     ██║   ██║██║   ██║
 ╚██████╗██║  ██║██║  ██║██║ ╚████║███████╗███████╗╚██████╔╝╚██████╔╝
  ╚═════╝╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═══╝╚══════╝╚══════╝ ╚═════╝  ╚═════╝
EOF
echo -e "${NC}"
echo -e "${WHITE}        VPN TUNNEL SCRIPT - XRAY/V2RAY EDITION${NC}"
echo -e "${YELLOW}        ══════════════════════════════════════${NC}"
echo ""

# ─── Input Domain ──────────────────────────────────────────
ask_domain() {
  while true; do
    echo -e "${CYAN}[*]${NC} Masukkan domain yang sudah diarahkan ke IP server ini:"
    echo -ne "  ${WHITE}Domain${NC}: "
    read -r DOMAIN
    DOMAIN=$(echo "$DOMAIN" | tr '[:upper:]' '[:lower:]' | sed 's/^[[:space:]]*//;s/[[:space:]]*$//')

    if [[ -z "$DOMAIN" ]]; then
      echo -e "${RED}[ERROR]${NC} Domain tidak boleh kosong!"
      continue
    fi

    if ! echo "$DOMAIN" | grep -qE '^([a-zA-Z0-9]([a-zA-Z0-9\-]{0,61}[a-zA-Z0-9])?\.)+[a-zA-Z]{2,}$'; then
      echo -e "${RED}[ERROR]${NC} Format domain tidak valid!"
      continue
    fi

    echo -e "${CYAN}[*]${NC} Memverifikasi domain ${WHITE}$DOMAIN${NC} → IP server..."
    SERVER_IP=$(curl -s4 --max-time 5 https://ifconfig.me 2>/dev/null || \
                curl -s4 --max-time 5 https://api.ipify.org 2>/dev/null || \
                curl -s4 --max-time 5 https://ipv4.icanhazip.com 2>/dev/null)
    DOMAIN_IP=$(dig +short "$DOMAIN" A 2>/dev/null | grep -E '^[0-9]+\.' | tail -1)

    if [[ -z "$SERVER_IP" ]]; then
      echo -e "${YELLOW}[WARN]${NC} Tidak bisa cek IP server, lanjut tanpa verifikasi..."
      break
    fi

    if [[ -z "$DOMAIN_IP" ]]; then
      echo -e "${RED}[WARN]${NC} DNS domain belum ditemukan."
      echo -ne "  Lanjutkan meski DNS belum propagasi? [y/N]: "
      read -r FORCE
      [[ "$FORCE" =~ ^[Yy]$ ]] && break
      continue
    fi

    if [[ "$DOMAIN_IP" == "$SERVER_IP" ]]; then
      echo -e "${GREEN}[OK]${NC} Domain ${WHITE}$DOMAIN${NC} → ${GREEN}$SERVER_IP${NC} ✓ VERIFIED"
      break
    else
      echo -e "${YELLOW}[WARN]${NC} Domain → $DOMAIN_IP, Server → $SERVER_IP (tidak cocok)"
      echo -ne "  Lanjutkan? [y/N]: "
      read -r FORCE
      [[ "$FORCE" =~ ^[Yy]$ ]] && break
    fi
  done
  echo "$DOMAIN" > /tmp/vpn_domain.tmp
}

# ─── Install Dependencies ──────────────────────────────────
install_deps() {
  echo -e "\n${CYAN}[*]${NC} Menginstall dependensi sistem..."
  apt-get update -qq 2>/dev/null
  apt-get install -y -qq \
    curl wget unzip zip socat tar \
    dnsutils net-tools \
    openssl ca-certificates \
    cron jq uuid-runtime \
    iptables build-essential \
    libssl-dev libpcre3-dev zlib1g-dev 2>/dev/null
  echo -e "${GREEN}[OK]${NC} Dependensi terinstall"
}

# ─── Install Nginx from chanelog/bin ──────────────────────
install_nginx() {
  echo -e "\n${CYAN}[*]${NC} Menginstall Nginx dari chanelog/bin..."

  # Coba dari chanelog/bin (nginx-1.28.0.tar.gz)
  wget -q "$REPO/nginx-1.28.0.tar.gz" -O /tmp/nginx-src.tar.gz 2>/dev/null
  if [[ -s /tmp/nginx-src.tar.gz ]]; then
    echo -e "${CYAN}[*]${NC} Build Nginx 1.28.0 dari source..."
    cd /tmp && tar xzf nginx-src.tar.gz 2>/dev/null
    local nginx_dir=$(find /tmp -maxdepth 1 -type d -name "nginx-*" | head -1)
    if [[ -d "$nginx_dir" ]]; then
      cd "$nginx_dir"
      ./configure --prefix=/etc/nginx \
        --sbin-path=/usr/sbin/nginx \
        --modules-path=/usr/lib64/nginx/modules \
        --conf-path=/etc/nginx/nginx.conf \
        --error-log-path=/var/log/nginx/error.log \
        --http-log-path=/var/log/nginx/access.log \
        --pid-path=/var/run/nginx.pid \
        --with-http_ssl_module \
        --with-http_v2_module \
        --with-http_realip_module \
        --with-http_stub_status_module 2>/dev/null
      make -j$(nproc) 2>/dev/null && make install 2>/dev/null
      cd /
      # Buat systemd service
      if [[ -f /usr/sbin/nginx ]]; then
        cat > /etc/systemd/system/nginx.service <<'EOF2'
[Unit]
Description=Nginx HTTP Server
After=network.target

[Service]
Type=forking
PIDFile=/var/run/nginx.pid
ExecStartPre=/usr/sbin/nginx -t
ExecStart=/usr/sbin/nginx
ExecReload=/bin/kill -s HUP $MAINPID
ExecStop=/bin/kill -s QUIT $MAINPID
PrivateTmp=true

[Install]
WantedBy=multi-user.target
EOF2
        mkdir -p /etc/nginx/conf.d /var/log/nginx /var/www/html
        systemctl daemon-reload
        systemctl enable nginx 2>/dev/null
        echo -e "${GREEN}[OK]${NC} Nginx 1.28.0 (dari chanelog/bin) terinstall"
        return
      fi
    fi
  fi

  # Fallback apt
  echo -e "${YELLOW}[WARN]${NC} Fallback ke apt untuk Nginx..."
  apt-get install -y -qq nginx 2>/dev/null
  systemctl enable nginx 2>/dev/null
  echo -e "${GREEN}[OK]${NC} Nginx terinstall (via apt)"
}

# ─── Install Dropbear from chanelog/bin ──────────────────
install_dropbear() {
  echo -e "\n${CYAN}[*]${NC} Menginstall Dropbear dari chanelog/bin..."

  # Coba dari chanelog/bin (dropbear-master.zip)
  wget -q "$REPO/dropbear-master.zip" -O /tmp/dropbear-src.zip 2>/dev/null
  if [[ -s /tmp/dropbear-src.zip ]]; then
    echo -e "${CYAN}[*]${NC} Menggunakan source dropbear dari chanelog/bin..."
    cd /tmp && unzip -qo dropbear-src.zip 2>/dev/null
    local db_dir=$(find /tmp -maxdepth 1 -type d -name "dropbear*" | head -1)
    if [[ -d "$db_dir" ]]; then
      cd "$db_dir"
      autoconf 2>/dev/null; autoheader 2>/dev/null
      ./configure --disable-zlib 2>/dev/null
      make -j$(nproc) PROGRAMS="dropbear dbclient dropbearkey dropbearconvert scp" 2>/dev/null
      make install 2>/dev/null
      cd /
    fi
  fi

  # Fallback apt jika binary belum ada
  if ! command -v dropbear &>/dev/null; then
    echo -e "${YELLOW}[WARN]${NC} Fallback ke apt untuk Dropbear..."
    apt-get install -y -qq dropbear 2>/dev/null
  fi

  cat > /etc/default/dropbear <<EOF2
NO_START=0
DROPBEAR_PORT=442
DROPBEAR_EXTRA_ARGS="-p 109 -p 143"
DROPBEAR_BANNER="/etc/vpn-script/banner.txt"
DROPBEAR_RECEIVE_WINDOW=65536
EOF2

  systemctl enable dropbear 2>/dev/null
  systemctl restart dropbear 2>/dev/null
  echo -e "${GREEN}[OK]${NC} Dropbear terinstall (port: 442, 109, 143)"
}

# ─── Install Xray ──────────────────────────────────────────
install_xray() {
  echo -e "\n${CYAN}[*]${NC} Menginstall Xray dari chanelog/bin..."
  mkdir -p /usr/local/bin /etc/xray /var/log/xray

  ARCH=$(uname -m)
  if [[ "$ARCH" == "aarch64" ]]; then
    XRAY_ZIP="Xray-linux-arm64-v8a.zip"
  else
    XRAY_ZIP="Xray-linux-64.zip"
  fi

  # Download dari chanelog/bin
  wget -q "$REPO/$XRAY_ZIP" -O /tmp/xray.zip 2>/dev/null
  if [[ $? -ne 0 ]] || [[ ! -s /tmp/xray.zip ]]; then
    echo -e "${YELLOW}[WARN]${NC} Gagal dari chanelog/bin, coba install-release.sh..."
    wget -q "$REPO/install-release.sh" -O /tmp/install-release.sh 2>/dev/null
    if [[ -s /tmp/install-release.sh ]]; then
      chmod +x /tmp/install-release.sh
      bash /tmp/install-release.sh 2>/dev/null
    else
      echo -e "${YELLOW}[WARN]${NC} Fallback ke GitHub release..."
      XRAY_VER=$(curl -s --max-time 10 https://api.github.com/repos/XTLS/Xray-core/releases/latest \
        | grep '"tag_name"' | cut -d'"' -f4)
      [[ -z "$XRAY_VER" ]] && XRAY_VER="v25.4.30"
      wget -q "https://github.com/XTLS/Xray-core/releases/download/${XRAY_VER}/${XRAY_ZIP}" \
        -O /tmp/xray.zip 2>/dev/null
    fi
  fi

  if [[ -s /tmp/xray.zip ]]; then
    cd /tmp && unzip -qo xray.zip xray 2>/dev/null
    [[ -f /tmp/xray ]] && mv /tmp/xray /usr/local/bin/xray
  fi

  if [[ ! -f /usr/local/bin/xray ]]; then
    echo -e "${RED}[ERROR]${NC} Xray binary tidak ditemukan! Install manual."
    exit 1
  fi

  chmod +x /usr/local/bin/xray

  # jq dari chanelog/bin
  if ! command -v jq &>/dev/null; then
    wget -q "$REPO/jq-linux-amd64" -O /usr/local/bin/jq 2>/dev/null
    chmod +x /usr/local/bin/jq 2>/dev/null
    command -v jq &>/dev/null || apt-get install -y -qq jq 2>/dev/null
  fi

  # Systemd service
  cat > /etc/systemd/system/xray.service <<EOF2
[Unit]
Description=Xray Service
Documentation=https://github.com/xtls
After=network.target nss-lookup.target

[Service]
User=root
CapabilityBoundingSet=CAP_NET_ADMIN CAP_NET_BIND_SERVICE
AmbientCapabilities=CAP_NET_ADMIN CAP_NET_BIND_SERVICE
NoNewPrivileges=true
ExecStart=/usr/local/bin/xray run -config /etc/xray/config.json
Restart=on-failure
RestartPreventExitStatus=23
LimitNOFILE=65536

[Install]
WantedBy=multi-user.target
EOF2

  systemctl daemon-reload
  systemctl enable xray
  echo -e "${GREEN}[OK]${NC} Xray terinstall ($(xray version 2>/dev/null | head -1))"
}

# ─── Install OHP Server from chanelog/bin ─────────────────
install_ohp() {
  echo -e "\n${CYAN}[*]${NC} Menginstall OHP Server dari chanelog/bin..."

  wget -q "$REPO/ohpserver" -O /usr/local/bin/ohpserver 2>/dev/null
  if [[ -s /usr/local/bin/ohpserver ]]; then
    chmod +x /usr/local/bin/ohpserver

    # Systemd service OHP (proxy ke port dropbear 442)
    cat > /etc/systemd/system/ohpserver.service <<EOF2
[Unit]
Description=OHP Server (HTTP Proxy Tunnel)
After=network.target

[Service]
ExecStart=/usr/local/bin/ohpserver -port 8080 -proxy 127.0.0.1:442
Restart=on-failure
RestartSec=5
LimitNOFILE=65536

[Install]
WantedBy=multi-user.target
EOF2

    systemctl daemon-reload
    systemctl enable ohpserver 2>/dev/null
    systemctl restart ohpserver 2>/dev/null
    echo -e "${GREEN}[OK]${NC} OHP Server terinstall (port: 8080 → SSH:442)"
  else
    echo -e "${YELLOW}[WARN]${NC} OHP Server gagal didownload dari chanelog/bin, skip."
  fi
}

# ─── Install BadVPN (UDP over TCP) from chanelog/bin ──────
install_badvpn() {
  echo -e "\n${CYAN}[*]${NC} Menginstall BadVPN (UDPGW) dari chanelog/bin..."

  wget -q "$REPO/badvpn-master.zip" -O /tmp/badvpn-src.zip 2>/dev/null
  if [[ -s /tmp/badvpn-src.zip ]]; then
    apt-get install -y -qq cmake 2>/dev/null
    cd /tmp && unzip -qo badvpn-src.zip 2>/dev/null
    local bv_dir=$(find /tmp -maxdepth 1 -type d -name "badvpn*" | head -1)
    if [[ -d "$bv_dir" ]]; then
      mkdir -p "$bv_dir/build" && cd "$bv_dir/build"
      cmake .. -DBUILD_NOTHING_BY_DEFAULT=1 -DBUILD_UDPGW=1 2>/dev/null
      make -j$(nproc) 2>/dev/null
      [[ -f udpgw/badvpn-udpgw ]] && mv udpgw/badvpn-udpgw /usr/local/bin/badvpn-udpgw
      cd /
    fi
  fi

  if [[ -f /usr/local/bin/badvpn-udpgw ]]; then
    chmod +x /usr/local/bin/badvpn-udpgw
    cat > /etc/systemd/system/badvpn.service <<EOF2
[Unit]
Description=BadVPN UDP Gateway
After=network.target

[Service]
ExecStart=/usr/local/bin/badvpn-udpgw --listen-addr 127.0.0.1:7300 --max-clients 500 --max-connections-for-client 100
Restart=on-failure
RestartSec=5

[Install]
WantedBy=multi-user.target
EOF2
    systemctl daemon-reload
    systemctl enable badvpn 2>/dev/null
    systemctl restart badvpn 2>/dev/null
    echo -e "${GREEN}[OK]${NC} BadVPN UDPGW terinstall (port: 7300)"
  else
    echo -e "${YELLOW}[WARN]${NC} BadVPN tidak berhasil di-build, skip."
  fi
}

# ─── Install Stunnel from chanelog/bin ────────────────────
install_stunnel() {
  echo -e "\n${CYAN}[*]${NC} Menginstall Stunnel dari chanelog/bin..."

  wget -q "$REPO/stunnel-master.zip" -O /tmp/stunnel-src.zip 2>/dev/null
  if [[ -s /tmp/stunnel-src.zip ]]; then
    cd /tmp && unzip -qo stunnel-src.zip 2>/dev/null
    local st_dir=$(find /tmp -maxdepth 1 -type d -name "stunnel*" | head -1)
    if [[ -d "$st_dir" ]]; then
      cd "$st_dir"
      autoreconf -fvi 2>/dev/null || true
      ./configure 2>/dev/null
      make -j$(nproc) 2>/dev/null && make install 2>/dev/null
      cd /
    fi
  fi

  if ! command -v stunnel &>/dev/null && ! command -v stunnel4 &>/dev/null; then
    echo -e "${YELLOW}[WARN]${NC} Fallback apt untuk Stunnel..."
    apt-get install -y -qq stunnel4 2>/dev/null
  fi

  if command -v stunnel &>/dev/null || command -v stunnel4 &>/dev/null; then
    mkdir -p /etc/stunnel
    # Config stunnel untuk SSL wrapping port SSH
    cat > /etc/stunnel/stunnel.conf <<EOF2
cert = /etc/ssl/xray/xray.crt
key  = /etc/ssl/xray/xray.key
socket = l:TCP_NODELAY=1
socket = r:TCP_NODELAY=1

[ssh-ssl]
accept  = 222
connect = 127.0.0.1:22
EOF2
    systemctl enable stunnel4 2>/dev/null || true
    systemctl restart stunnel4 2>/dev/null || true
    echo -e "${GREEN}[OK]${NC} Stunnel terinstall (SSL-SSH port: 222)"
  else
    echo -e "${YELLOW}[WARN]${NC} Stunnel tidak berhasil diinstall, skip."
  fi
}

# ─── Install Trojan-Go from chanelog/bin ──────────────────
install_trojan() {
  echo -e "\n${CYAN}[*]${NC} Menginstall Trojan-Go dari chanelog/bin..."

  wget -q "$REPO/trojan-go-linux-amd64.zip" -O /tmp/trojan-go.zip 2>/dev/null
  if [[ -s /tmp/trojan-go.zip ]]; then
    mkdir -p /etc/trojan-go /var/log/trojan-go
    cd /tmp && unzip -qo trojan-go.zip 2>/dev/null
    [[ -f /tmp/trojan-go ]] && mv /tmp/trojan-go /usr/local/bin/trojan-go
    chmod +x /usr/local/bin/trojan-go 2>/dev/null

    if [[ -f /usr/local/bin/trojan-go ]]; then
      # Config trojan-go dasar (aktif setelah SSL tersedia)
      cat > /etc/trojan-go/config.json <<EOF2
{
  "run_type": "server",
  "local_addr": "0.0.0.0",
  "local_port": 444,
  "remote_addr": "127.0.0.1",
  "remote_port": 80,
  "password": ["changeme123"],
  "ssl": {
    "cert": "/etc/ssl/xray/xray.crt",
    "key": "/etc/ssl/xray/xray.key",
    "sni": ""
  },
  "websocket": {
    "enabled": true,
    "path": "/trojan-ws",
    "host": ""
  },
  "log_level": 2,
  "log_file": "/var/log/trojan-go/access.log"
}
EOF2

      cat > /etc/systemd/system/trojan-go.service <<EOF2
[Unit]
Description=Trojan-Go Proxy Server
After=network.target

[Service]
User=root
ExecStart=/usr/local/bin/trojan-go -config /etc/trojan-go/config.json
Restart=on-failure
RestartSec=5
LimitNOFILE=65536

[Install]
WantedBy=multi-user.target
EOF2
      systemctl daemon-reload
      systemctl enable trojan-go 2>/dev/null
      echo -e "${GREEN}[OK]${NC} Trojan-Go terinstall (port: 444, password default: changeme123)"
      echo -e "${YELLOW}[INFO]${NC} Ganti password Trojan-Go di /etc/trojan-go/config.json"
    fi
  else
    echo -e "${YELLOW}[WARN]${NC} Trojan-Go gagal didownload dari chanelog/bin, skip."
  fi
}

# ─── Install Fail2Ban from chanelog/bin ───────────────────
install_fail2ban() {
  echo -e "\n${CYAN}[*]${NC} Menginstall Fail2Ban dari chanelog/bin..."

  wget -q "$REPO/fail2ban-master.zip" -O /tmp/fail2ban-src.zip 2>/dev/null
  if [[ -s /tmp/fail2ban-src.zip ]]; then
    apt-get install -y -qq python3 python3-setuptools 2>/dev/null
    cd /tmp && unzip -qo fail2ban-src.zip 2>/dev/null
    local fb_dir=$(find /tmp -maxdepth 1 -type d -name "fail2ban*" | head -1)
    if [[ -d "$fb_dir" ]]; then
      cd "$fb_dir"
      python3 setup.py install 2>/dev/null
      cd /
    fi
  fi

  if ! command -v fail2ban-server &>/dev/null; then
    echo -e "${YELLOW}[WARN]${NC} Fallback apt untuk Fail2Ban..."
    apt-get install -y -qq fail2ban 2>/dev/null
  fi

  if command -v fail2ban-server &>/dev/null; then
    cat > /etc/fail2ban/jail.local <<EOF2
[DEFAULT]
bantime  = 3600
findtime = 600
maxretry = 5
backend  = systemd

[sshd]
enabled = true
port    = 22,442,109,143
logpath = %(sshd_log)s

[nginx-http-auth]
enabled = true
EOF2
    systemctl enable fail2ban 2>/dev/null
    systemctl restart fail2ban 2>/dev/null
    echo -e "${GREEN}[OK]${NC} Fail2Ban terinstall & aktif (ban 1 jam setelah 5x gagal login)"
  else
    echo -e "${YELLOW}[WARN]${NC} Fail2Ban tidak berhasil diinstall, skip."
  fi
}

# ─── Install VnStat from chanelog/bin ─────────────────────
install_vnstat() {
  echo -e "\n${CYAN}[*]${NC} Menginstall VnStat dari chanelog/bin..."

  wget -q "$REPO/vnstat-master.zip" -O /tmp/vnstat-src.zip 2>/dev/null
  if [[ -s /tmp/vnstat-src.zip ]]; then
    apt-get install -y -qq libsqlite3-dev 2>/dev/null
    cd /tmp && unzip -qo vnstat-src.zip 2>/dev/null
    local vn_dir=$(find /tmp -maxdepth 1 -type d -name "vnstat*" | head -1)
    if [[ -d "$vn_dir" ]]; then
      cd "$vn_dir"
      ./configure --prefix=/usr 2>/dev/null
      make -j$(nproc) 2>/dev/null && make install 2>/dev/null
      cd /
    fi
  fi

  if ! command -v vnstat &>/dev/null; then
    echo -e "${YELLOW}[WARN]${NC} Fallback apt untuk VnStat..."
    apt-get install -y -qq vnstat 2>/dev/null
  fi

  if command -v vnstat &>/dev/null; then
    systemctl enable vnstat 2>/dev/null
    systemctl restart vnstat 2>/dev/null
    echo -e "${GREEN}[OK]${NC} VnStat terinstall (monitor traffic real-time)"
  else
    echo -e "${YELLOW}[WARN]${NC} VnStat tidak berhasil diinstall, skip."
  fi
}

# ─── Configure Nginx ───────────────────────────────────────
configure_nginx() {
  local domain="$1"
  mkdir -p /etc/nginx/conf.d /var/www/html

  rm -f /etc/nginx/sites-enabled/default
  rm -f /etc/nginx/conf.d/default.conf

  cat > /etc/nginx/conf.d/xray.conf <<EOF2
# Port 80 — non-TLS WebSocket + redirect ke HTTPS untuk request biasa
server {
    listen 80;
    listen [::]:80;
    server_name ${domain};

    location /vmess-ntls {
        proxy_redirect off;
        proxy_pass http://127.0.0.1:10003;
        proxy_http_version 1.1;
        proxy_set_header Upgrade \$http_upgrade;
        proxy_set_header Connection "upgrade";
        proxy_set_header Host \$host;
        proxy_set_header X-Real-IP \$remote_addr;
        proxy_set_header X-Forwarded-For \$proxy_add_x_forwarded_for;
        proxy_connect_timeout 60s;
        proxy_read_timeout 3600s;
    }

    location /vless-ntls {
        proxy_redirect off;
        proxy_pass http://127.0.0.1:10004;
        proxy_http_version 1.1;
        proxy_set_header Upgrade \$http_upgrade;
        proxy_set_header Connection "upgrade";
        proxy_set_header Host \$host;
        proxy_set_header X-Real-IP \$remote_addr;
        proxy_set_header X-Forwarded-For \$proxy_add_x_forwarded_for;
        proxy_connect_timeout 60s;
        proxy_read_timeout 3600s;
    }

    location /trojan-ws {
        proxy_redirect off;
        proxy_pass http://127.0.0.1:444;
        proxy_http_version 1.1;
        proxy_set_header Upgrade \$http_upgrade;
        proxy_set_header Connection "upgrade";
        proxy_set_header Host \$host;
        proxy_set_header X-Real-IP \$remote_addr;
        proxy_set_header X-Forwarded-For \$proxy_add_x_forwarded_for;
        proxy_connect_timeout 60s;
        proxy_read_timeout 3600s;
    }

    location / {
        return 301 https://\$host\$request_uri;
    }
}

# Port 443 — HTTPS/TLS WebSocket
server {
    listen 443 ssl http2;
    listen [::]:443 ssl http2;
    server_name ${domain};

    ssl_certificate /etc/ssl/xray/xray.crt;
    ssl_certificate_key /etc/ssl/xray/xray.key;
    ssl_protocols TLSv1.2 TLSv1.3;
    ssl_ciphers ECDHE-ECDSA-AES128-GCM-SHA256:ECDHE-RSA-AES128-GCM-SHA256:ECDHE-ECDSA-AES256-GCM-SHA384:ECDHE-RSA-AES256-GCM-SHA384;
    ssl_prefer_server_ciphers off;
    ssl_session_cache shared:SSL:10m;
    ssl_session_timeout 1d;

    location /vmess-ws {
        proxy_redirect off;
        proxy_pass http://127.0.0.1:10001;
        proxy_http_version 1.1;
        proxy_set_header Upgrade \$http_upgrade;
        proxy_set_header Connection "upgrade";
        proxy_set_header Host \$host;
        proxy_set_header X-Real-IP \$remote_addr;
        proxy_set_header X-Forwarded-For \$proxy_add_x_forwarded_for;
        proxy_connect_timeout 60s;
        proxy_read_timeout 3600s;
    }

    location /vless-ws {
        proxy_redirect off;
        proxy_pass http://127.0.0.1:10002;
        proxy_http_version 1.1;
        proxy_set_header Upgrade \$http_upgrade;
        proxy_set_header Connection "upgrade";
        proxy_set_header Host \$host;
        proxy_set_header X-Real-IP \$remote_addr;
        proxy_set_header X-Forwarded-For \$proxy_add_x_forwarded_for;
        proxy_connect_timeout 60s;
        proxy_read_timeout 3600s;
    }

    location / {
        root /var/www/html;
        index index.html;
    }
}
EOF2

  cat > /var/www/html/index.html <<EOF2
<!DOCTYPE html>
<html>
<head><title>${domain}</title></head>
<body style="background:#1a1a2e;color:#e0e0e0;font-family:monospace;text-align:center;padding:50px">
<h1 style="color:#00d4ff">Server Running</h1>
</body>
</html>
EOF2

  nginx -t 2>/dev/null && systemctl restart nginx 2>/dev/null
  echo -e "${GREEN}[OK]${NC} Nginx dikonfigurasi"
}

# ─── Generate Xray Config ──────────────────────────────────
generate_xray_config() {
  mkdir -p /etc/vpn-script/db
  touch /etc/vpn-script/db/vmess.db
  touch /etc/vpn-script/db/vless.db

  cat > /etc/xray/config.json <<'XRAYEOF'
{
  "log": {
    "loglevel": "warning",
    "error": "/var/log/xray/error.log",
    "access": "/var/log/xray/access.log"
  },
  "inbounds": [
    {
      "tag": "vmess-ws-tls",
      "port": 10001,
      "listen": "127.0.0.1",
      "protocol": "vmess",
      "settings": { "clients": [] },
      "streamSettings": {
        "network": "ws",
        "wsSettings": { "path": "/vmess-ws" }
      }
    },
    {
      "tag": "vless-ws-tls",
      "port": 10002,
      "listen": "127.0.0.1",
      "protocol": "vless",
      "settings": { "clients": [], "decryption": "none" },
      "streamSettings": {
        "network": "ws",
        "wsSettings": { "path": "/vless-ws" }
      }
    },
    {
      "tag": "vmess-ws-ntls",
      "port": 10003,
      "listen": "127.0.0.1",
      "protocol": "vmess",
      "settings": { "clients": [] },
      "streamSettings": {
        "network": "ws",
        "wsSettings": { "path": "/vmess-ntls" }
      }
    },
    {
      "tag": "vless-ws-ntls",
      "port": 10004,
      "listen": "127.0.0.1",
      "protocol": "vless",
      "settings": { "clients": [], "decryption": "none" },
      "streamSettings": {
        "network": "ws",
        "wsSettings": { "path": "/vless-ntls" }
      }
    }
  ],
  "outbounds": [
    { "protocol": "freedom", "settings": {} }
  ]
}
XRAYEOF

  echo -e "${GREEN}[OK]${NC} Xray config dibuat"
}

# ─── Install SSL via acme.sh ───────────────────────────────
install_ssl() {
  local domain="$1"
  echo -e "\n${CYAN}[*]${NC} Menginstall acme.sh dari chanelog/bin..."
  mkdir -p /etc/ssl/xray

  wget -q "$REPO/acme.sh" -O /tmp/acme_install.sh 2>/dev/null
  if [[ -s /tmp/acme_install.sh ]]; then
    chmod +x /tmp/acme_install.sh
    bash /tmp/acme_install.sh --install-online -m "admin@${domain}" 2>/dev/null || \
    bash /tmp/acme_install.sh -m "admin@${domain}" 2>/dev/null
  fi

  if [[ ! -f /root/.acme.sh/acme.sh ]]; then
    echo -e "${YELLOW}[WARN]${NC} Fallback install acme.sh dari official..."
    curl -s https://get.acme.sh | bash -s email="admin@${domain}" 2>/dev/null
  fi

  if [[ ! -f /root/.acme.sh/acme.sh ]]; then
    echo -e "${RED}[ERROR]${NC} acme.sh gagal diinstall!"
    echo -e "${YELLOW}[WARN]${NC} Membuat self-signed cert sementara..."
    openssl req -x509 -newkey ec -pkeyopt ec_paramgen_curve:P-256 \
      -keyout /etc/ssl/xray/xray.key \
      -out /etc/ssl/xray/xray.crt \
      -days 365 -nodes \
      -subj "/CN=${domain}" 2>/dev/null
    return
  fi

  systemctl stop nginx 2>/dev/null
  sleep 1

  echo -e "${CYAN}[*]${NC} Meminta SSL certificate untuk ${WHITE}$domain${NC}..."

  /root/.acme.sh/acme.sh --register-account -m "admin@${domain}" 2>/dev/null

  /root/.acme.sh/acme.sh --issue \
    --standalone \
    -d "$domain" \
    --keylength ec-256 \
    --httpport 80 \
    --force 2>/dev/null

  if [[ $? -ne 0 ]]; then
    echo -e "${YELLOW}[WARN]${NC} Let's Encrypt gagal, coba ZeroSSL..."
    /root/.acme.sh/acme.sh --set-default-ca --server zerossl 2>/dev/null
    /root/.acme.sh/acme.sh --issue \
      --standalone \
      -d "$domain" \
      --keylength ec-256 \
      --httpport 80 \
      --force 2>/dev/null
  fi

  /root/.acme.sh/acme.sh --installcert -d "$domain" \
    --ecc \
    --key-file /etc/ssl/xray/xray.key \
    --fullchain-file /etc/ssl/xray/xray.crt \
    --reloadcmd "systemctl restart xray nginx trojan-go 2>/dev/null" 2>/dev/null

  chmod 644 /etc/ssl/xray/xray.key 2>/dev/null
  chmod 644 /etc/ssl/xray/xray.crt 2>/dev/null

  if [[ ! -s /etc/ssl/xray/xray.crt ]]; then
    echo -e "${YELLOW}[WARN]${NC} SSL gagal, membuat self-signed cert sementara..."
    openssl req -x509 -newkey ec -pkeyopt ec_paramgen_curve:P-256 \
      -keyout /etc/ssl/xray/xray.key \
      -out /etc/ssl/xray/xray.crt \
      -days 365 -nodes \
      -subj "/CN=${domain}" 2>/dev/null
  fi

  systemctl start nginx 2>/dev/null
  echo -e "${GREEN}[OK]${NC} SSL Certificate selesai untuk $domain"
}

# ─── Download & Install Script Files ──────────────────────
install_script_files() {
  local domain="$1"

  echo -e "\n${CYAN}[*]${NC} Mendownload script files dari GitHub..."
  mkdir -p $SCRIPT_DIR/db $SCRIPT_DIR/menu

  echo "$domain" > $SCRIPT_DIR/domain
  echo "$domain" > $SCRIPT_DIR/db/domain

  local ok=true
  for f in menu.sh lib.sh; do
    echo -ne "  Downloading $f..."
    wget -q --timeout=30 "$RAW/$f" -O "$SCRIPT_DIR/$f"
    if [[ $? -ne 0 ]] || [[ ! -s "$SCRIPT_DIR/$f" ]]; then
      echo -e " ${RED}GAGAL${NC}"
      ok=false
    else
      echo -e " ${GREEN}OK${NC}"
    fi
  done

  for f in vmess vless nginx dropbear sysinfo changedomain uninstall; do
    echo -ne "  Downloading menu/${f}.sh..."
    wget -q --timeout=30 "$RAW/menu/${f}.sh" -O "$SCRIPT_DIR/menu/${f}.sh"
    if [[ $? -ne 0 ]] || [[ ! -s "$SCRIPT_DIR/menu/${f}.sh" ]]; then
      echo -e " ${RED}GAGAL${NC}"
      ok=false
    else
      echo -e " ${GREEN}OK${NC}"
    fi
  done

  if [[ "$ok" == "false" ]]; then
    echo -e "${RED}[ERROR]${NC} Beberapa file gagal didownload!"
    echo -e "${YELLOW}[INFO]${NC} Pastikan repo https://github.com/fauzanihanipah/ray sudah berisi semua file"
    exit 1
  fi

  chmod +x $SCRIPT_DIR/*.sh
  chmod +x $SCRIPT_DIR/menu/*.sh

  ln -sf $SCRIPT_DIR/menu.sh $BIN_DIR/vpn
  chmod +x $BIN_DIR/vpn

  # Simpan info tools yang terinstall dari chanelog/bin
  cat > $SCRIPT_DIR/installed_tools.conf <<EOF2
# Tools dari chanelog/bin yang terinstall
XRAY_BIN=$(command -v xray 2>/dev/null || echo "not installed")
OHP_BIN=$(command -v ohpserver 2>/dev/null || echo "not installed")
BADVPN_BIN=$(command -v badvpn-udpgw 2>/dev/null || echo "not installed")
TROJAN_BIN=$(command -v trojan-go 2>/dev/null || echo "not installed")
FAIL2BAN=$(command -v fail2ban-server 2>/dev/null || echo "not installed")
VNSTAT=$(command -v vnstat 2>/dev/null || echo "not installed")
STUNNEL=$(command -v stunnel 2>/dev/null || command -v stunnel4 2>/dev/null || echo "not installed")
EOF2

  cat > $SCRIPT_DIR/banner.txt <<'EOF2'
  ╔══════════════════════════════════════╗
  ║     CHANELOG VPN TUNNEL SERVER       ║
  ║     Unauthorized access prohibited  ║
  ╚══════════════════════════════════════╝
EOF2

  echo -e "${GREEN}[OK]${NC} Script files terinstall"
}

# ─── Setup Cron ────────────────────────────────────────────
setup_cron() {
  crontab -l 2>/dev/null | grep -v "vpn-script\|acme.sh --cron" | crontab -
  (crontab -l 2>/dev/null; echo "0 0 * * * bash $SCRIPT_DIR/lib.sh delete_expired >> /var/log/vpn-cleanup.log 2>&1") | crontab -
  (crontab -l 2>/dev/null; echo "0 3 * * * /root/.acme.sh/acme.sh --cron >> /var/log/acme-renew.log 2>&1") | crontab -
  echo -e "${GREEN}[OK]${NC} Cron jobs dikonfigurasi"
}

# ─── Firewall ──────────────────────────────────────────────
setup_firewall() {
  # Port standar + port tool tambahan dari chanelog/bin
  for port in 22 80 443 109 143 442 8080 444 222 7300; do
    iptables -I INPUT -p tcp --dport $port -j ACCEPT 2>/dev/null
  done
  # UDP untuk BadVPN
  iptables -I INPUT -p udp --dport 7300 -j ACCEPT 2>/dev/null
  mkdir -p /etc/iptables
  iptables-save > /etc/iptables/rules.v4 2>/dev/null
  echo -e "${GREEN}[OK]${NC} Firewall dikonfigurasi"
}

# ─── Main ──────────────────────────────────────────────────
main() {
  clear
  echo -e "${CYAN}══════════════════════════════════════════════════${NC}"
  echo -e "${WHITE}      CHANELOG VPN SCRIPT - PROSES INSTALASI     ${NC}"
  echo -e "${CYAN}══════════════════════════════════════════════════${NC}"
  echo ""

  ask_domain
  DOMAIN=$(cat /tmp/vpn_domain.tmp)
  rm -f /tmp/vpn_domain.tmp

  echo -e "\n${YELLOW}[INFO]${NC} Domain : ${WHITE}$DOMAIN${NC}"
  echo -ne "  Lanjutkan instalasi? [Y/n]: "
  read -r CONFIRM
  [[ "$CONFIRM" =~ ^[Nn]$ ]] && exit 0

  install_deps
  install_nginx
  install_dropbear
  install_xray
  install_ohp
  install_badvpn
  install_stunnel
  install_trojan
  install_fail2ban
  install_vnstat
  generate_xray_config
  configure_nginx "$DOMAIN"
  install_ssl "$DOMAIN"
  install_script_files "$DOMAIN"
  setup_cron
  setup_firewall

  # Update trojan-go config dengan domain yang benar
  if [[ -f /etc/trojan-go/config.json ]]; then
    sed -i "s/\"sni\": \"\"/\"sni\": \"$DOMAIN\"/" /etc/trojan-go/config.json
    sed -i "s/\"host\": \"\"/\"host\": \"$DOMAIN\"/" /etc/trojan-go/config.json
  fi

  # Update stunnel config dengan domain
  if [[ -f /etc/stunnel/stunnel.conf ]]; then
    # Restart stunnel setelah SSL ada
    systemctl restart stunnel4 2>/dev/null || true
  fi

  # Restart semua service
  systemctl daemon-reload 2>/dev/null
  systemctl restart xray 2>/dev/null
  systemctl restart nginx 2>/dev/null
  systemctl restart dropbear 2>/dev/null
  systemctl restart ohpserver 2>/dev/null
  systemctl restart badvpn 2>/dev/null
  systemctl restart trojan-go 2>/dev/null
  systemctl restart fail2ban 2>/dev/null
  systemctl restart vnstat 2>/dev/null

  echo ""
  echo -e "${GREEN}╔══════════════════════════════════════════════════╗${NC}"
  echo -e "${GREEN}║       ✓  INSTALASI BERHASIL SELESAI!             ║${NC}"
  echo -e "${GREEN}╠══════════════════════════════════════════════════╣${NC}"
  echo -e "${GREEN}║${NC}  Domain      : ${WHITE}$DOMAIN${NC}"
  echo -e "${GREEN}║${NC}  Xray        : $(systemctl is-active xray 2>/dev/null)"
  echo -e "${GREEN}║${NC}  Nginx       : $(systemctl is-active nginx 2>/dev/null)"
  echo -e "${GREEN}║${NC}  Dropbear    : $(systemctl is-active dropbear 2>/dev/null)"
  echo -e "${GREEN}║${NC}  OHP Server  : $(systemctl is-active ohpserver 2>/dev/null)"
  echo -e "${GREEN}║${NC}  BadVPN UDPGW: $(systemctl is-active badvpn 2>/dev/null)"
  echo -e "${GREEN}║${NC}  Stunnel     : $(systemctl is-active stunnel4 2>/dev/null)"
  echo -e "${GREEN}║${NC}  Trojan-Go   : $(systemctl is-active trojan-go 2>/dev/null)"
  echo -e "${GREEN}║${NC}  Fail2Ban    : $(systemctl is-active fail2ban 2>/dev/null)"
  echo -e "${GREEN}║${NC}  VnStat      : $(systemctl is-active vnstat 2>/dev/null)"
  echo -e "${GREEN}╠══════════════════════════════════════════════════╣${NC}"
  echo -e "${GREEN}║${NC}  Port SSH    : 22, 442, 109, 143"
  echo -e "${GREEN}║${NC}  Port SSL-SSH: 222 (Stunnel)"
  echo -e "${GREEN}║${NC}  Port OHP    : 8080"
  echo -e "${GREEN}║${NC}  Port Trojan : 444"
  echo -e "${GREEN}║${NC}  Port UDPGW  : 7300 (BadVPN)"
  echo -e "${GREEN}╠══════════════════════════════════════════════════╣${NC}"
  echo -e "${GREEN}║${NC}  Jalankan   : ${CYAN}vpn${NC}"
  echo -e "${GREEN}╚══════════════════════════════════════════════════╝${NC}"
  echo ""

  bash $SCRIPT_DIR/menu.sh
}

main "$@"
